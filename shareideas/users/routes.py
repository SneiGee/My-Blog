...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash('Your account is created! you can now log in', 'success')
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            ...
            flash('Login failed! Please check password and email', 'danger')
...
def request_reset():
    ...
    if form.validate_on_submit():
        ...
        flash('An email messsage is sent to you so follow the instruction to reset you password', 'success')
        return redirect(url_for('users.login'))
